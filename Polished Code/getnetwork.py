...
lines = [x.rstrip('\n') for x in open('500_tweets.csv','r')]
blah = []
...
f = open('groupsntwork_noah2.csv', 'w')
writer = csv.writer(f)

#for each tweet loop thru and get the retweeters of the tweet
timeout = 0
for item in blah:
    ...
    if retweeters != None:
        for user in retweeters:
            #write the line containing the tweet id and the user id to a line
