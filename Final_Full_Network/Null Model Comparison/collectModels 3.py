...
def directed_connected_double_edge_swap_weighted(G, nswap=1, _window_threshold=3, seed=None):
    ...
    and creates the new edges u->x and v->y::
     ...
     u-->v           u-->y
            becomes  
     x-->y           x-->v
     edge weight from u-->v becomes the edge weight for u-->y
     
     edge weight from x-->y becomes the edge weight for x-->v
...
#                     #print("sametarget")
                ...
                #edge weight from u-->v becomes the edge weight for u-->y
                uv_weight = G[u][v]["weight"] 
                #print("uv_weight:")
                #print(uv_weight)
                #edge weight from x-->y becomes the edge weight for x-->v
                xy_weight = G[x][y]["weight"]     
                #print("xy_weight:")
                #print(xy_weight)
                if (v not in G[x]) and (y not in G[u]):  # don't create parallel edges
                    if (u == y or x ==v): # dont create a self loop
                        #print("No self loops swap not made")
                        #print("______________________")
                        continue
                    G.add_edge(u, y)
                    G[u][y]["weight"] = uv_weight
                    
                    #print("edge_added: ")
                    #print(u)
                    #print("->")
                    #print(y)
                    #print("u->y edge weight:")
                    #print(G[u][y]["weight"] )
                    G.add_edge(x, v)
                    G[x][v]["weight"] = xy_weight
                    #print("edge_added: ")
                    #print(x)
                    #print("->")
                    #print(v)
                    #print("x->v edge weight:")
                    #print(G[x][v]["weight"] )
                ...
                # Otherwise, undo the changes.
                    ...
                    G.add_edge(u, v)
                    G[u][v]["weight"] = uv_weight
                    G.add_edge(x, y)
                    G[x][y]["weight"] = xy_weight
        ...
        # check if the graph remains connected.
            ...
            while wcount < window and n < nswap:
                ...
                if (v not in G[x]) and (y not in G[u]):  # don't create parallel edges
                    ...
                    #print(y)
                    #print("______________________")
                    swapped.append((u, v, x, y, uv_weight, xy_weight))
                    swapcount += 1
                #else:
                    #print("Swap not made")
                    #print("______________________")
            ...
            # the window size.
                ...
                while swapped:
                    (u, v, x, y, uv_weight, xy_weight) = swapped.pop()
                    G.add_edge(u, v)
                    G.add_edge(x, y)
                    G[u][v]["weight"] = uv_weight
                    G[x][y]["weight"] = xy_weight
...
for i in range(400,600):
    H = G.copy()
    directed_connected_double_edge_swap_weighted(H, nswap=20*len( G.edges() ))
    nx.write_weighted_edgelist(H, "Directed_Weighted_Models/Directed_Weighted_Random_Null_Model_{}".format(i))
