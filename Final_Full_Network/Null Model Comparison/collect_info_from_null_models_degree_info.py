...
deg_pd = "{}_deg_df.csv".format(groupname)
in_deg_pd = "{}_out_deg_df.csv".format(groupname)
out_deg_pd = "{}_in_deg_df.csv".format(groupname)
...
for file in os.listdir(directory):
    ...
    if (count == 0):
        ...
        persist_first_to_csv(nx.degree(G),deg_pd,filename)
        persist_first_to_csv(nx.in_degree(G),in_deg_pd,filename)
        persist_first_to_csv(nx.out_degree(G),out_deg_pd,filename)
    else:
        persist_centrality_to_csv(nx.degree(G),deg_pd,filename)
        persist_centrality_to_csv(nx.in_degree(G),in_deg_pd,filename)
        persist_centrality_to_csv(nx.out_degree(G),out_deg_pd,filename)
    count = count + 1
